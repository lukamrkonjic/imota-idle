...
## rain streaks and A-Short-Hike wind lines. Rendered INSIDE the low-res pixel viewport (attached in
## world.gd via WorldRender3D.attach_pixel_overlay), so its flakes/streaks share the world's pixel
## grid — chunky + on-grid — instead of floating as thin full-res lines on top. It draws in the
## (internal) viewport space and ignores the 2D camera. Sizes/counts are tuned for the LOW internal
## resolution (~500-670px); each drawn pixel becomes a chunky block after the nearest-neighbour
## upscale. Intensity is read live from the Weather autoload — this node only renders.

const SNOW_MAX := 60    # particle counts tuned for the small internal buffer (keeps on-screen density)
const RAIN_MAX := 44
const WIND_MAX := 14
...
func _draw_snow(size: Vector2, amt: float, wind: float) -> void:
	for i: int in int(round(amt * SNOW_MAX)):
		...
		var y := life * (size.y + 12.0) - 6.0
		var x := _wrap(rx * size.x + sin(_t * 0.9 + rp * 31.0) * (2.0 + wind * 8.0) + life * wind * 32.0, size.x)
		var s := 1.0 + rx * 1.5   # 1-3 internal px -> chunky flakes after upscale
		var col := Color(0.95, 0.97, 1.0, (0.55 + 0.4 * amt) * (0.65 + 0.35 * sin(life * PI)))
		# Snap to the internal pixel grid so flakes never straddle a pixel (stable, crisp).
		draw_rect(Rect2(floorf(x), floorf(y), s, s), col)
...
func _draw_rain(size: Vector2, amt: float, wind: float) -> void:
	...
	for i: int in int(round(amt * RAIN_MAX)):
		...
		var p := Vector2(floorf(_wrap(rx * size.x + life * wind * 55.0, size.x)), floorf(life * (size.y + 14.0) - 7.0))
		var col := Color(0.66, 0.76, 0.96, 0.34 + 0.26 * amt)
		draw_line(p, p - dir * (5.0 + rx * 7.0), col, 1.0 + rx * 0.6)   # short chunky streaks (internal px)
...
func _draw_wind(size: Vector2, wind: float) -> void:
	...
	for i: int in int(round(clampf(wind - 0.28, 0.0, 1.0) * WIND_MAX)):
		...
		var p := Vector2(-size.x * 0.2 + life * size.x * 1.4, floorf(ry * size.y + sin(life * TAU + rp * 9.0) * 4.0))
		var col := Color(1.0, 1.0, 1.0, (0.12 + 0.22 * wind) * sin(life * PI))   # clearer streaks
		draw_line(p, p + dir * (26.0 + rl * 56.0), col, 1.0 + rl * 0.7)          # long thin chunky wind line
