...
static func draw(canvas: CanvasItem, variant: int = 0) -> void:
	...
	var hh := hw * 0.5
	# plinth block
	PixelDraw.iso_block_tex(canvas, 0.0, 0.0, hw + 4.0, (hw + 4.0) * 0.5, 9.0, PixelPalette.shade(stone, 0.9))
	# fluted shaft prism on top of the plinth
	var sh := h - 9.0
	PixelDraw.iso_block_tex(canvas, 0.0, -9.0, hw, hh, sh, stone)
	...
	if variant % 3 == 0:
		...
		PixelDraw.iso_block_tex(canvas, 0.0, top_y, hw + 2.5, (hw + 2.5) * 0.5, 6.0, PixelPalette.shade(stone, 1.02))
	else:
		# jagged broken crown — a smaller block knocked off-centre
		PixelDraw.iso_block_tex(canvas, 1.5, top_y, hw * 0.72, hw * 0.36, 5.0, PixelPalette.shade(stone, 0.84))
